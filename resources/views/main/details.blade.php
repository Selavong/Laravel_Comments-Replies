                    ...
                    <div class="col-12 col-xl-8">
                        ...
                        <div class="article__content">
                            ...
                            <ul class="list">
                                ...
                                <li>Action</li>
                                <li>{{ $movie->realease }}</li>
                                <li>{{ $movie->runtime }} </li>
                                <li>{{ $movie->age }}+</li>
                            </ul>

                            <p>{{ $movie->diskfreespace }}</p>
                        </div>
                        <!-- end article content -->
                    </div>
                                        ...
                                        {{-- {{$comment}} --}}
                                            <li class="comments__item">
                                                <div class="comments__autor">
                                                    <img class="comments__avatar" src="{{ $comment->user->profile ? asset('profile-photos/' . $comment->user->profile) : asset('profile-photos/no-profile.jpg') }}" alt="Profile Image">

                                                    <span class="comments__name">{{ $comment->user['name'] }}</span>
                                                    <span
                                                        class="comments__time">{{ \Carbon\Carbon::parse($comment->created_at)->format('d-m-Y, H:i') }}
                                                    </span>
                                                    @if (Auth::user()->name == $comment->user['name'])
                                                        ...
                                                        <div class="comments__actions">
                                                            <span
                                                                ...
                                                                <i class="fa fa-pen"
                                                                    style="color:rgb(80, 255, 153);cursor: pointer; font-size: 20px;"
                                                                    aria-hidden="true"></i>
                                                            </span>
                                                            <span onclick="deleteComment('{{ $comment->id }}')">
                                                                <i class="fa fa-trash"
                                                                    style="color:rgb(255, 80, 80);cursor: pointer; font-size: 20px;"
                                                                    aria-hidden="true"></i>
                                                            </span>
                                                ...
                                                <p class="comments__text comments__text-{{ $comment->id }}">
                                                    {{ $comment->content }}
                                                </p>
                                                <div class="sign__group" id="edit-form-{{ $comment->id }}"
                                                    style="display: none;">
                                                    <textarea class="sign__textarea" id="edit-content-{{ $comment->id }}"></textarea>
                                                    <div class="col-12 text-end">
                                                        <button class="sign__btn col-3"
                                                ...
                                                <div class="reply-form" id="reply-form-{{ $comment->id }}"
                                                    ...
                                                    <form action="{{ route('comments.reply', $comment->id) }}"
                                                        method="POST" class="comments__form">
                                                        @csrf
                                                        <div class="pb-3">
                                                            <input name="reply_content" class="sign__input"
                                                                placeholder="Write your reply..." input="">
                                                            <!-- Assuming you want to associate the movie_id with the reply -->
                                                ...
                                                <ul class="comments__list">
                                                    @foreach ($comment->replies as $reply)
                                                        <li class="comments__item">
                                                            <div class="comments__autor">
                                                                <img class="comments__avatar" src="{{ $reply->user->profile ? asset('profile-photos/' . $reply->user->profile) : asset('profile-photos/no-profile.jpg') }}" alt="Profile Image">
                                                                <span
                                                                    class="comments__name">{{ $reply->user['name'] }}</span>
                                                                <span
                                                                    class="comments__time">{{ \Carbon\Carbon::parse($reply->created_at)->format('d-m-Y, H:i') }}</span>
                                                                @if (Auth::check() && Auth::user()->name == $reply->user['name'])
                                                                    <div class="comments__actions">
                                                                        <span
                                                                            onclick="showEditReplyForm({{ $reply->id }}, '{{ $reply->content }}')"
                                                                            style="padding-right: 20px;">
                                                                            <i class="fa fa-pen"
                                                                                style="color: rgb(80, 255, 153); cursor: pointer; font-size: 20px;"
                                                                                aria-hidden="true"></i>
                                                            ...
                                                            <p class="comments__text comments__text-{{ $reply->id }}">
                                                                {{ $reply->content }}
                                                            </p>
                                                            <div class="sign__group"
                                                                id="edit-reply-form-{{ $reply->id }}"
                                                                style="display: none;">
                                                                <textarea class="sign__textarea" id="edit-reply-content-{{ $reply->id }}"></textarea>
                                                                <div class="col-12 text-end">
                                                                    <button class="sign__btn col-3"
                                                                        onclick="updateReply({{ $reply->id }})">Update</button>
                                                                    <button class="sign__btn col-3"
                                                                        onclick="hideEditReplyForm({{ $reply->id }})">Cancel</button>
                                                                </div>
                                                            </div>
                                                        </li>
                                                    @endforeach
                                    ...
                                    <ul class="reviews__list">
                                        <li class="reviews__item">
                                            <div class="reviews__autor">
                                                ...
                                                <span class="reviews__name">Best Marvel movie in my
                                                    opinion</span>
                                                <span class="reviews__time">24.08.2021, 17:53 by Jonathan
                                                    Banks</span>
                                            ...
                                            <p class="reviews__text">There are many variations of passages of
                                                Lorem Ipsum
                                        ...
                                        <li class="reviews__item">
                                            <div class="reviews__autor">
                                                ...
                                                <span class="reviews__name">Best Marvel movie in my
                                                    opinion</span>
                                                <span class="reviews__time">24.08.2021, 17:53 by Jesse
                                                    Plemons</span>
                                                <span class="reviews__rating"><svg xmlns="http://www.w3.org/2000/svg"
                                                        viewBox="0 0 24 24">
                                                        <path
                                        ...
                                        <li class="reviews__item">
                                            <div class="reviews__autor">
                                                ...
                                                <span class="reviews__name">Best Marvel movie in my
                                                    opinion</span>
                                                <span class="reviews__time">24.08.2021, 17:53 by Charles
                                                    Baker</span>
                                                <span class="reviews__rating"><svg xmlns="http://www.w3.org/2000/svg"
                                                        viewBox="0 0 24 24">
                                                        <path
                                    ...
                                    <form action="#" class="reviews__form">
                                        <div class="row">
                                            ...
                                            <form action="{{ route('comments.store') }}" method="POST"
                                                ...
                                                <div class="sign__group">
                                                    <textarea id="text" name="content" class="sign__textarea" placeholder="Add comment"></textarea>
                                                    <input value="{{ $movie->id }}" id="movie_id" name="movie_id"
                                                        type="hidden" />
                                                </div>
                                                <button type=" submit" class="sign__btn">Send</button>
                                            </form>
                            ...
                            <div class="row">
                                <div class="col-12">
                                    <form action="#" class="subscribe">
                                        ...
                                        <p class="subscribe__text">Subscribe to notifications about new episodes
                                        </p>
                                        <div class="sign__group">
...
@section('customJs')
    <script>
        ...
        const deleteCommentUrl = '{{ route('comments.destroy', ':id') }}';
        ...
        function deleteComment(commentId) {
            ...
            $.ajax({
                url: deleteCommentUrl.replace(':id', commentId),
                type: 'DELETE',
                data: {
                    _token: csrfToken
                },
                success: function(response) {
                    ...
                    location.href = response.movie_id; // Make sure response contains the movie ID
                },
        ...
        function showEditForm(commentId, content) {
            ...
            $(`.comments__text-${commentId}`).hide();

            // Show the edit form
            $(`#edit-form-${commentId}`).show();
            $(`#edit-content-${commentId}`).val(content);
        }

        function hideEditForm(commentId) {
            // Show the comment text
            $(`.comments__text-${commentId}`).show();

            // Hide the edit form
            $(`#edit-form-${commentId}`).hide();
            $(`#edit-content-${commentId}`).val("");
        }

        function updateComment(commentId) {
            const content = $(`#edit-content-${commentId}`).val();
            const csrfToken = $('meta[name="csrf-token"]').attr('content');
        ...
        function showEditReplyForm(replyId, content) {
            // Hide the reply text
            $(`.comments__text-${replyId}`).hide();

            // Show the edit form
            $(`#edit-reply-form-${replyId}`).show();
            $(`#edit-reply-content-${replyId}`).val(content);
        }

        // Hide edit reply form
        function hideEditReplyForm(replyId) {
            // Show the reply text
            $(`.comments__text-${replyId}`).show();

            // Hide the edit form
            $(`#edit-reply-form-${replyId}`).hide();
            $(`#edit-reply-content-${replyId}`).val("");
        }

        // Update reply
        function updateReply(replyId) {
            const content = $(`#edit-reply-content-${replyId}`).val();
            const csrfToken = $('meta[name="csrf-token"]').attr('content');
            ...
            $.ajax({
                ...
                success: function(response) {
                    // Reload or update the replies section
                    location.reload(); // Example: Reload the page after update
                },
                error: function(xhr, status, error) {
                    console.error('Error:', error);
